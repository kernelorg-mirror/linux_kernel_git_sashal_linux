...
static int __init cma_activate_fake_area(struct cma *cma)
{
	...
	printk("\033[31mFunction = %s, successfully init a fake cma area, pfn: 0x%lX, length: 0x%lX\033[m\n", __PRETTY_FUNCTION__, (unsigned int)cma->base_pfn, (unsigned int)cma->count);
	return 0;
}
#endif
			...
			bool fixed, const char *name, struct cma **res_cma)
{
	...
	if(strncmp(name, "fakemem", 7) == 0)
	{
		printk("\033[31mFunction = %s, Line = %d, base_bus_addr 0x%lX is not valid, fakemem passing reserving memory_block\033[m\n", __PRETTY_FUNCTION__, __LINE__, base);
		goto pass_reserve;
	}
#endif
