...
#if defined(CONFIG_MP_ASYM_UMA_ALLOCATION) && defined(CONFIG_ARM64)
...
zonelist_scan:
	...
	if (gfp_mask & __GFP_DMA || mma_dma_zone_size == 0) { // DMA specified or symmetric UMA
		int i = 0;
	...
	} else if (ac->zonelist->zone_sym && ac->zonelist->zone_asym) {
		...
		panic("%s,zone_sym=0x%x, zone_asym=0x%x\n", __FUNCTION__, ac->zonelist->zone_sym, ac->zonelist->zone_asym);
	}
