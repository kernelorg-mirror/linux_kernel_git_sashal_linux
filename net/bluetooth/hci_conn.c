				       ...
				       u8 role, u16 handle)
{
	...
	case LE_LINK:
		...
		conn->le_rx_def_phys = hdev->le_rx_def_phys;
		break;
	case ISO_LINK:
		/* conn->src should reflect the local identity address */
		hci_copy_identity_address(hdev, &conn->src, &conn->src_type);
			      ...
			      __u8 base_len, __u8 *base, u16 timeout)
{
	...
	conn = hci_add_bis(hdev, dst, qos, base_len, eir, timeout);
	if (IS_ERR(conn))
		return conn;
				 ...
				 __u8 base_len, __u8 *base, u16 timeout)
{
	...
	conn = hci_bind_bis(hdev, dst, qos, base_len, base, timeout);
	if (IS_ERR(conn))
		return conn;
