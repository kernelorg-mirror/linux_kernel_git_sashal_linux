...
/**
 * batadv_get_meshif_from_info() - Retrieve mesh interface from genl attributes
...
batadv_get_meshif_from_info(struct net *net, struct genl_info *info)
{
	struct net_device *mesh_iface;
	int ifindex;
			   ...
			   struct genl_info *info)
{
	...
	if (ops->internal_flags & BATADV_FLAG_NEED_MESH) {
		mesh_iface = batadv_get_meshif_from_info(net, info);
