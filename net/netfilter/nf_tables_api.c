				       ...
				       struct nft_hook_dump_ctx *dump_ctx)
{
	...
	if (nla_put(skb, NFTA_DEVICE_NAME, hook->ifnamelen, hook->ifname))
		return -1;
	...
	if (nft_base_chain_netdev(family, ops->hooknum)) {
		...
		if (dump_hook_ctx.n == 1 &&
		    nla_put(skb, NFTA_HOOK_DEV,
			    dump_hook_ctx.first->ifnamelen,
			    dump_hook_ctx.first->ifname))
			goto nla_put_failure;
	}
	nla_nest_end(skb, nest);
	...
	list_for_each_entry_safe(trans, next, &nft_net->commit_list, list) {
		...
		case NFT_MSG_DESTROYCHAIN:
			if (nft_trans_chain_update(trans)) {
				...
				nf_tables_chain_notify(&ctx, NFT_MSG_DELCHAIN,
						       NULL, NULL);
				nf_tables_unregister_hook(ctx.net,
							  table,
							  ctx.chain);
			}
			break;
		case NFT_MSG_NEWRULE:
			nft_clear(ctx.net, nft_trans_rule(trans));
			nf_tables_rule_notify(&ctx,
