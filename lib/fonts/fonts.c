...
/*
 * Helpers for font data
 */

static struct font_data *to_font_data_struct(const unsigned char *fd)
{
	return container_of(fd, struct font_data, data[0]);
}

static bool font_data_is_internal(const unsigned char *fd)
{
	return !REFCOUNT(fd); /* internal fonts have no reference counting */
}

static void font_data_free(const unsigned char *fd)
{
