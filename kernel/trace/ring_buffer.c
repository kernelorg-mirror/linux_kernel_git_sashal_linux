		...
		long nr_pages, struct list_head *pages)
{
	...
	for (i = 0; i < nr_pages; i++) {
		...
		/*
		 * Use __GFP_RETRY_MAYFAIL here (unlike alloc_cpu_page())
		 * since this is the bulk allocation path guarded by
		 * set_current_oom_origin() above: it must keep failing
		 * gracefully instead of invoking the oom-killer.
		 */
		bpage = (struct buffer_page *)
			kzalloc_node(ALIGN(sizeof(*bpage), cache_line_size()),
		...
		bpage->page = alloc_cpu_data(cpu_buffer->cpu, cpu_buffer->buffer->subbuf_order);
		if (!bpage->page)
			goto free_pages;
...
rb_allocate_cpu_buffer(struct trace_buffer *buffer, long nr_pages, int cpu)
{
	...
	bpage->page = alloc_cpu_data(cpu, cpu_buffer->buffer->subbuf_order);
	if (!bpage->page)
			  ...
			  size_t len, int cpu, int full)
{
	...
	if (size < buffer->subbuf_size)
		memset(&bpage->data[size], 0, buffer->subbuf_size - size);
