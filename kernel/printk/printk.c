...
/**
 * sys_syslog - Read and control the kernel log buffer
 * @type:	Command type specifying the operation to perform
 * @buf:	User buffer for reading kernel messages
 * @len:	Buffer size or console log level depending on command
 *
 * long-desc: Provides access to the kernel printk ring buffer and control
 *   over console logging behavior. This syscall has multiple operating modes
 *   selected by the @type parameter. The kernel maintains a circular buffer
 *   of messages generated by printk(); this syscall allows reading those
 *   messages and controlling their delivery to the console. The glibc
 *   wrapper function is klogctl(). Note this is completely different from
 *   the userspace syslog(3) function that communicates with syslogd.
 *   Commands 0 (SYSLOG_ACTION_CLOSE) and 1 (SYSLOG_ACTION_OPEN) are
 *   historical no-ops retained for compatibility. Command 2
 *   (SYSLOG_ACTION_READ) blocks until messages are available, reads them
 *   destructively (messages read are removed from further reads via this
 *   command), and is typically used by /proc/kmsg. Commands 3 and 4 read
 *   non-destructively (3) or read-and-clear (4) from a "cleared" sequence
 *   marker. Command 5 advances this clear marker without reading. Commands
 *   6-8 control console logging levels. Commands 9-10 query buffer sizes.
 *
 ...
 *   range: 0, 10
 *   constraint: Must be a valid SYSLOG_ACTION_* command (0-10). Commands:
 *     0 (SYSLOG_ACTION_CLOSE) - No-op, returns 0.
 *     1 (SYSLOG_ACTION_OPEN) - No-op, returns 0.
 *     2 (SYSLOG_ACTION_READ) - Read and consume messages, blocks if empty.
 *     3 (SYSLOG_ACTION_READ_ALL) - Non-destructively read all messages.
 *     4 (SYSLOG_ACTION_READ_CLEAR) - Read all and advance clear marker.
 *     5 (SYSLOG_ACTION_CLEAR) - Advance clear marker (affects 3,4 only).
 *     6 (SYSLOG_ACTION_CONSOLE_OFF) - Disable console logging.
 *     7 (SYSLOG_ACTION_CONSOLE_ON) - Re-enable console logging.
 *     8 (SYSLOG_ACTION_CONSOLE_LEVEL) - Set console log level to @len.
 *     9 (SYSLOG_ACTION_SIZE_UNREAD) - Return bytes available for command 2.
 *     10 (SYSLOG_ACTION_SIZE_BUFFER) - Return total log buffer size.
 *
 * param: buf
 *   type: KAPI_TYPE_USER_PTR
 *   flags: KAPI_PARAM_OUT | KAPI_PARAM_USER | KAPI_PARAM_OPTIONAL
 *   constraint-type: KAPI_CONSTRAINT_CUSTOM
 *   constraint: Required for commands 2, 3, 4 where messages are read.
 *     Must be a valid user-space pointer to a buffer of at least @len
 *     bytes. May be NULL for commands 0, 1, 5, 6, 7, 8, 9, 10 where no
 *     data is returned in the buffer. When required, a NULL pointer
 *     results in -EINVAL. Buffer receives kernel log messages formatted
 *     with optional timestamps depending on printk_time setting.
 *
 * param: len
 *   type: KAPI_TYPE_INT
 *   flags: KAPI_PARAM_IN
 *   constraint-type: KAPI_CONSTRAINT_CUSTOM
 *   constraint: Meaning varies by command. For commands 2, 3, 4: buffer
 *     size in bytes, must be >= 0 (0 returns immediately with 0). For
 *     command 8: console log level, must be in range 1-8 (values below
 *     minimum_console_loglevel are clamped up). For other commands: ignored.
 *     Negative values for read commands return -EINVAL.
 *
 * return:
 *   type: KAPI_TYPE_INT
 *   check-type: KAPI_RETURN_ERROR_CHECK
 *   success: >= 0
 *   desc: Return value depends on command. Commands 0, 1, 5, 6, 7, 8:
 *     return 0 on success. Commands 2, 3, 4: return number of bytes read
 *     into @buf (may be 0 for command 2 only if len was 0, otherwise
 *     command 2 blocks until at least one byte is available). Command 9:
 *     return number of bytes available to read via command 2. Command 10:
 *     return total kernel log buffer size (configurable via LOG_BUF_SHIFT).
 *     On error, returns negative errno value.
 *
 * error: EINVAL, Invalid command or parameters
 *   desc: Returned when: (1) @type is not a valid command (not 0-10),
 *     (2) for commands 2, 3, 4: @buf is NULL or @len is negative,
 *     (3) for command 8: @len is outside range 1-8. Note that @len of 0
 *     for read commands is valid and returns 0 immediately.
 *
 * error: EPERM, Permission denied by capability check
 *   desc: The operation requires CAP_SYSLOG capability and the calling
 *     process does not have it. Required for commands 0, 1, 2, 4, 5, 6,
 *     7, 8 unless /proc/sys/kernel/dmesg_restrict is 0 (default on most
 *     systems). Commands 3 (READ_ALL) and 10 (SIZE_BUFFER) are allowed
 *     unprivileged unless dmesg_restrict is set to 1. Note: access from
 *     /proc/kmsg checks capability at open() time, not per-read.
 *
 * error: EACCES, Permission denied by LSM security module
 *   desc: A Linux Security Module (SELinux, AppArmor, etc.) denied the
 *     operation. SELinux checks system:syslog_read for commands 3 and 10,
 *     system:syslog_console for commands 6, 7, 8, and system:syslog_mod
 *     for all other commands. This is checked after capability checks.
 *
 * error: EFAULT, Bad user-space buffer address
 *   desc: For commands 2, 3, 4: the @buf pointer fails access_ok() check,
 *     or copy_to_user() fails when writing data to the buffer. This
 *     indicates @buf points to unmapped or read-only memory.
 *
 * error: ENOMEM, Memory allocation failed
 *   desc: For commands 2, 3, 4: failed to allocate temporary kernel buffer
 *     (PRINTK_MESSAGE_MAX bytes, typically 8KB) for formatting messages
 *     before copying to user space. This is a transient condition that
 *     may succeed on retry after memory is freed.
 *
 * error: ERESTARTSYS, Interrupted by signal
 *   desc: For command 2 (READ) only: the syscall was blocked waiting for
 *     new messages and was interrupted by a signal. The syscall may be
 *     automatically restarted by the kernel if SA_RESTART is set for the
 *     interrupting signal. No data was read. User space sees EINTR if
 *     the syscall is not restarted.
 *
 * error: ENOSYS, Syscall not implemented
 *   desc: The kernel was compiled with CONFIG_PRINTK disabled. All
 *     operations fail with this error. This configuration is rare,
 *     typically only used in deeply embedded systems.
 *
 * lock: syslog_lock
 *   type: KAPI_LOCK_MUTEX
 *   acquired: true
 *   released: true
 *   desc: Mutex protecting syslog_seq (read position for command 2),
 *     syslog_partial (partial message state), and clear_seq (clear
 *     position for commands 4, 5). Acquired during command 2 (READ)
 *     around the wait and read loops, command 4/5 clear operations,
 *     and command 9 (SIZE_UNREAD) calculation. Lock is released while
 *     waiting for new messages in command 2 and while copying to user
 *     space, then reacquired.
 *
 * signal: any
 *   direction: KAPI_SIGNAL_RECEIVE
 *   action: KAPI_SIGNAL_ACTION_RETURN
 *   condition: Only for command 2 (SYSLOG_ACTION_READ) while blocked
 *     waiting for new kernel messages to become available.
 *   desc: Command 2 uses wait_event_interruptible() which can be
 *     interrupted by any non-blocked signal. When interrupted, returns
 *     -ERESTARTSYS which the kernel may convert to -EINTR or restart
 *     automatically depending on signal disposition and SA_RESTART flag.
 *   error: -ERESTARTSYS
 *   timing: KAPI_SIGNAL_TIME_BLOCKING
 *   restartable: yes
 *
 * side-effect: KAPI_EFFECT_MODIFY_STATE
 *   target: Console log level (console_loglevel global)
 *   desc: Commands 6 and 7 save/restore the console log level. Command 6
 *     saves current level and sets to minimum_console_loglevel (silencing
 *     most output). Command 7 restores the saved level. Command 8 sets
 *     a specific level (1-8, clamped to minimum) and clears saved state.
 *   condition: Commands 6, 7, or 8
 *   reversible: yes
 *
 * side-effect: KAPI_EFFECT_MODIFY_STATE
 *   target: Syslog read position (syslog_seq)
 *   desc: Command 2 (READ) advances syslog_seq as messages are consumed.
 *     This is the only command that removes messages from the reader's
 *     view; other readers via /proc/kmsg share this position.
 *   condition: Command 2 successfully reads messages
 *   reversible: no
 *
 * side-effect: KAPI_EFFECT_MODIFY_STATE
 *   target: Clear sequence marker (clear_seq)
 *   desc: Commands 4 (READ_CLEAR) and 5 (CLEAR) advance clear_seq to
 *     the current end of the log buffer. This affects what messages
 *     are visible to subsequent commands 3 and 4, but does not affect
 *     command 2 or the actual ring buffer contents.
 *   condition: Commands 4 or 5
 *   reversible: no
 *
 * side-effect: KAPI_EFFECT_ALLOC_MEMORY
 *   target: Temporary kernel buffer
 *   desc: Commands 2, 3, 4 allocate a temporary buffer (PRINTK_MESSAGE_MAX
 *     bytes) with kmalloc(GFP_KERNEL) for formatting messages before
 *     copying to user space. Buffer is freed before syscall returns.
 *   condition: Commands 2, 3, or 4
 *   reversible: yes
 *
 * capability: CAP_SYSLOG
 *   type: KAPI_CAP_GRANT_PERMISSION
 *   allows: Access to restricted syslog operations
 *   without: Commands 3 and 10 allowed if dmesg_restrict == 0;
 *     all other commands return -EPERM without this capability.
 *   condition: Checked for restricted commands based on dmesg_restrict
 *     sysctl setting. When dmesg_restrict == 0 (default), only commands
 *     3 (READ_ALL) and 10 (SIZE_BUFFER) are unrestricted. When
 *     dmesg_restrict == 1, all commands require CAP_SYSLOG.
 *
 * constraint: dmesg_restrict sysctl
 *   desc: The /proc/sys/kernel/dmesg_restrict sysctl (default 0) controls
 *     whether unprivileged users can read kernel logs. When 0, commands
 *     3 and 10 are available without CAP_SYSLOG. When 1, all commands
 *     require CAP_SYSLOG. This was added in Linux 2.6.37 along with the
 *     CAP_SYSLOG capability separation from CAP_SYS_ADMIN.
 *   expr: dmesg_restrict == 0 || capable(CAP_SYSLOG)
 *
 * constraint: Console log level bounds
 *   desc: For command 8 (CONSOLE_LEVEL), the @len parameter must be 1-8.
 *     Values below minimum_console_loglevel (usually 1) are clamped up.
 *     The kernel config CONSOLE_LOGLEVEL_DEFAULT sets initial level.
 *   expr: 1 <= len && len <= 8
 *
 * constraint: Ring buffer overflow
 *   desc: The kernel log ring buffer is finite (size from LOG_BUF_SHIFT,
 *     typically 128KB-1MB). Old messages are overwritten when full.
 *     Command 2 readers may miss messages if they cannot keep up. When
 *     a reader's position points to overwritten messages, it is advanced
 *     to the oldest available message.
 *
 * examples: klogctl(SYSLOG_ACTION_READ_ALL, buf, sizeof(buf));  // Read all logs
 *   klogctl(SYSLOG_ACTION_CONSOLE_OFF, NULL, 0);  // Silence console
 *   klogctl(SYSLOG_ACTION_SIZE_BUFFER, NULL, 0);  // Get buffer size
 *
 * notes: Before Linux 2.6.37, CAP_SYS_ADMIN was required instead of
 *   CAP_SYSLOG. The split was made to allow containers to be denied
 *   syslog access while retaining other admin capabilities. Command 5
 *   (CLEAR) does not actually clear the ring buffer; it only advances
 *   a marker that affects commands 3 and 4. Command 2 is unaffected by
 *   command 5. The /proc/kmsg interface uses the same underlying
 *   mechanism as command 2, with capability checks at open() time. This
 *   syscall is the basis for dmesg(1) utility. When called from
 *   /proc/kmsg read handlers, the capability check is bypassed for all
 *   commands except SYSLOG_ACTION_OPEN because it was done at open().
 *
 * since-version: 1.0
 */
