...
Load prompts from the ai-review/ directory at the kernel tree root and from
ai-review/ subdirectories within each subsystem's source tree.  If a prompt
directory is not provided, assume it is the same directory as the prompt file.
...
Read `subsystem-index.md` and load all matching subsystem guides and critical patterns.
...
Scan the diff against all triggers in `subsystem-index.md` and load matching files
IMMEDIATELY.
