...
2. The prompt directory path (contains `agent/` and pattern files; subsystem guides are distributed under their respective subsystem directories)
...
2. **Run failed-review subagent**:
   ...
   Task(
     ...
     prompt: """
       ...
       You may need to update or create subsystem guides in <subsystem>/ai-review/
       directories based on the classification of missed bugs.
     """
   )
   ```
...
3. **Check for changes and commit if needed**:
   ...
   Look for:
   - **Modified files**: `modified: */ai-review/*.md` or `modified: ai-review/subsystem-index.md`
   - **New files**: `Untracked files: ... */ai-review/*.md`

   If there are ANY changes (modified OR new files) in `*/ai-review/` directories:
   ```bash
   cd <prompt_dir>/.. && \
     git add -A '*/ai-review/*.md' ai-review/subsystem-index.md && \
   ...
   - **New subsystem guide created**: e.g., `drivers/gpu/drm/ai-review/drm.md` appears as untracked
   - **Existing guide updated**: e.g., `fs/btrfs/ai-review/btrfs.md` appears as modified
   - **Trigger table updated**: `ai-review/subsystem-index.md` modified to add new entry
...
Note: subsystem guides are distributed under `<subsystem>/ai-review/` directories
(e.g., `kernel/locking/ai-review/locking.md`, `net/ai-review/networking.md`).
...
**Directory layout**:
```
ai-review/                          (prompt_dir)
├── agent/
│   ├── failed-review.md
│   ├── process-failed-reviews.md   (this file)
│   └── ...
├── subsystem-index.md              (trigger table)
├── subsystem-template.md           (guide format spec)
├── technical-patterns.md
└── ...

Subsystem guides (distributed):
├── kernel/locking/ai-review/locking.md
├── kernel/rcu/ai-review/rcu.md
├── net/ai-review/networking.md
├── drivers/gpu/drm/ai-review/drm.md
└── ...
