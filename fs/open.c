...
/**
 * sys_openat - Open or create a file relative to a directory file descriptor
 * @dfd: Directory file descriptor for relative path resolution, or AT_FDCWD
 * @filename: Pathname of the file to open or create (absolute or relative)
 * @flags: File access mode and behavior flags (O_RDONLY, O_WRONLY, O_RDWR, etc.)
 * @mode: File permission bits for newly created files (only with O_CREAT/O_TMPFILE)
 *
 * long-desc: Opens the file specified by filename, resolving relative paths
 *   starting from the directory referred to by dfd instead of the current
 *   working directory. If dfd is the special value AT_FDCWD (-100), relative
 *   paths are resolved from the current working directory (equivalent to open()).
 *   Absolute paths (starting with '/') ignore dfd entirely.
 *
 *   The dfd parameter enables race-free filesystem traversal by maintaining a
 *   stable reference to a directory even if it is renamed or moved. This prevents
 *   TOCTOU (time-of-check-time-of-use) vulnerabilities that can occur with
 *   traditional path-based operations. It also enables implementing per-thread
 *   working directories in multi-threaded applications.
 *
 *   If O_CREAT or O_TMPFILE is specified in flags, the file is created if it
 *   does not exist; its mode is set according to the mode parameter modified
 ...
 *   The return value is a file descriptor, a small nonnegative integer used in
 *   subsequent system calls (read, write, lseek, fcntl, etc.) to refer to the
 *   open file. The file descriptor returned by a successful openat is the lowest-
 *   numbered file descriptor not currently open for the process.
 *
 *   On 64-bit systems, O_LARGEFILE is automatically added to the flags. On 32-bit
 *   systems, files larger than 2GB require O_LARGEFILE to be explicitly set.
 *
 *   For modern code requiring additional path resolution control (e.g., restricting
 *   symlink traversal, mount point crossing), use openat2() with resolve flags.
 *
 * context-flags: KAPI_CTX_PROCESS | KAPI_CTX_SLEEPABLE
 *
 * param: dfd
 ...
 *   flags: KAPI_PARAM_IN
 *   constraint-type: KAPI_CONSTRAINT_CUSTOM
 *   constraint: Must be AT_FDCWD (-100) to use current working directory, or a
 *     valid file descriptor referring to a directory. The directory must have
 *     been opened with O_RDONLY, O_RDWR, or O_PATH. When filename is an absolute
 *     path (starts with '/'), dfd is ignored. If dfd refers to a non-directory
 *     file and filename is relative, ENOTDIR is returned.
 *
 * param: filename
 *   type: KAPI_TYPE_PATH
 *   flags: KAPI_PARAM_IN | KAPI_PARAM_USER
 ...
 *   constraint: Must be a valid null-terminated path string in user memory.
 *     Maximum path length is PATH_MAX (4096 bytes) including null terminator.
 *     For relative paths, resolution starts from the directory referred to by
 *     dfd (or cwd if dfd is AT_FDCWD). For absolute paths, resolution starts
 *     from the root directory and dfd is ignored. The path is followed (symlinks
 *     resolved) unless O_NOFOLLOW is specified. Empty strings return ENOENT
 *     unless AT_EMPTY_PATH is used with openat2().
 *
 * param: flags
 *   type: KAPI_TYPE_INT
 *   flags: KAPI_PARAM_IN
 ...
 *
 * error: EBADF, Bad file descriptor
 *   desc: dfd is not a valid open file descriptor and is not AT_FDCWD, or
 *     dfd was opened with O_PATH and an I/O operation was attempted.
 *
 * error: EBUSY, Device or resource busy
 *   desc: O_EXCL was specified in flags and pathname refers to a block device
 *     that is in use by the system (e.g., it is mounted).
 ...
 *   desc: The call was interrupted by a signal handler before completing file
 *     open. This can occur during lock acquisition or when breaking leases.
 *     The syscall is restartable with SA_RESTART.
 *
 * error: EINVAL, Invalid argument
 *   desc: Returned for several conditions: (1) Invalid O_* flag combinations
 *     (O_DIRECTORY|O_CREAT, O_TMPFILE without O_DIRECTORY, O_TMPFILE with
 ...
 * error: ENOTDIR, Not a directory
 *   desc: A component used as a directory in pathname is not actually a directory,
 *     or O_DIRECTORY was specified and pathname was not a directory, or dfd
 *     refers to a file other than a directory and pathname is a relative path.
 *
 * error: ENXIO, No such device or address
 *   desc: O_NONBLOCK | O_WRONLY is set and the named file is a FIFO and no
 *     process has the FIFO open for reading. Also returned when opening a device
 ...
 *   desc: pathname refers to a regular file that is too large to be opened.
 *     This occurs on 32-bit systems without O_LARGEFILE when the file size
 *     exceeds 2GB (2^31 - 1 bytes). Also returned when uid/gid mapping fails.
 *
 * error: EPERM, Operation not permitted
 *   desc: O_NOATIME flag was specified but the effective UID of the caller did
 *     not match the owner of the file and the caller is not privileged, or the
 ...
 *     (4096 bytes). Individual path components must not exceed NAME_MAX (255).
 *
 * examples: fd = openat(AT_FDCWD, "/etc/passwd", O_RDONLY);  // Like open()
 *   fd = openat(dirfd, "subdir/file.txt", O_RDONLY);  // Relative to dirfd
 *   fd = openat(dirfd, "../other/file", O_RDONLY);  // Can go up with ..
 *   fd = openat(dirfd, "/absolute/path", O_RDONLY);  // Ignores dirfd
 *   fd = openat(dirfd, "newfile", O_WRONLY | O_CREAT | O_EXCL, 0600);  // Create
 *   fd = openat(dirfd, ".", O_RDONLY | O_DIRECTORY);  // Open dirfd itself
 *   fd = openat(dirfd, "", O_RDONLY | O_PATH | O_NOFOLLOW);  // Via openat2
 *
 * notes: openat() was introduced to address race conditions in path-based
 *   operations. Traditional open() with relative paths is vulnerable to TOCTOU
 *   attacks where a malicious process can manipulate the directory tree between
 *   successive calls. Using openat() with a held directory fd provides a stable
 *   reference point immune to such manipulation.
 *
 *   The distinction between O_RDONLY, O_WRONLY, and O_RDWR is critical.
 *   O_RDONLY is defined as 0, so (flags & O_RDONLY) will be true for all flags.
 *   Test access mode using (flags & O_ACCMODE) == O_RDONLY.
 *
 *   When O_CREAT is specified without O_EXCL, there is a race condition between
 ...
 *   I/O operations will fail with EBADF.
 *
 *   The dfd can itself be obtained via openat(), allowing construction of
 *   arbitrary reference points in the filesystem hierarchy. Opening a directory
 *   with O_PATH is sufficient for use as dfd, requiring only execute permission
 *   on the directory components, not read permission on the directory itself.
 *
 * since-version: 2.6.16
 */
