...
/**
 * sys_lgetxattr - Retrieve an extended attribute value without following symlinks
 * @pathname: Path to the file from which to retrieve the attribute
 * @name: Null-terminated name of the extended attribute (includes namespace prefix)
 * @value: Buffer to receive the attribute value, or NULL for size query
 * @size: Size of the value buffer in bytes, or 0 to query current size
 *
 * long-desc: Retrieves the value of an extended attribute identified by name
 *   from the file specified by pathname. This syscall is identical to getxattr()
 *   except that if pathname refers to a symbolic link, lgetxattr() retrieves
 *   the attributes of the link itself, rather than the file it refers to.
 *
 *   Extended attributes are name:value pairs associated with inodes (files,
 *   directories, symbolic links, etc.) that extend the normal attributes
 *   (stat data) associated with all inodes.
 ...
 *   attribute value without copying it to the buffer. This can be used to
 *   determine an appropriate buffer size for a subsequent call. However, the
 *   attribute value may change between calls (race condition with lsetxattr).
 *
 *   If size is non-zero and less than the actual attribute size, ERANGE is
 *   returned and no data is copied.
 *
 *   Note that the user.* namespace is NOT supported on symbolic links. Even
 *   though lgetxattr() operates on the symlink itself, attempting to read
 *   user.* attributes from a symbolic link returns ENODATA. This is because
 *   the user.* namespace is restricted to regular files and directories. The
 *   trusted.* and security.* namespaces work on all file types including
 *   symbolic links.
 *
 * context-flags: KAPI_CTX_PROCESS | KAPI_CTX_SLEEPABLE
 *
 ...
 *   constraint-type: KAPI_CONSTRAINT_USER_PATH
 *   constraint: Must be a valid null-terminated path string in user memory.
 *     The path is resolved WITHOUT following the final symbolic link component
 *     (AT_SYMLINK_NOFOLLOW is used). However, symbolic links in directory
 *     components of the path ARE followed. Maximum path length is PATH_MAX
 *     (4096 bytes). The caller must have search permission on all directory
 ...
 * error: ENODATA, Attribute not found
 *   desc: The named extended attribute does not exist on the file. Also returned
 *     when attempting to read user.* attributes from symbolic links (since the
 *     user.* namespace is only supported on regular files and directories). Also
 *     returned when reading trusted.* namespace without CAP_SYS_ADMIN capability.
 *     For symlinks, only security.* and trusted.* (with capability) namespaces
 *     can be read.
 *
 * error: ENOENT, File not found
 *   desc: The file specified by pathname does not exist, or a directory component
 *     in the path does not exist. Note that the final component is not required
 *     to exist as a regular file - lgetxattr can retrieve attributes from
 *     symbolic links, but the link itself must exist.
 *
 * error: ERANGE, Buffer too small
 *   desc: The size parameter is non-zero but smaller than the actual attribute
 *     value size. Also returned if the attribute name is empty (zero length) or
 ...
 * error: ELOOP, Too many symbolic links
 *   desc: Too many symbolic links were encountered during path resolution
 *     of directory components (more than MAXSYMLINKS, typically 40). Note that
 *     the final path component is never followed, but preceding symlinks are.
 *
 * error: ENAMETOOLONG, Filename too long
 *   desc: The pathname or a component of the pathname exceeds the system limit
 *     (PATH_MAX or NAME_MAX).
 ...
 * constraint: user.* namespace restrictions on symlinks
 *   desc: The user.* namespace is only supported on regular files and directories.
 *     Since lgetxattr operates on symbolic links directly, attempting to read
 *     user.* attributes from a symlink always returns ENODATA, even if the
 *     symlink's target is a regular file or directory with that attribute.
 *     This is a deliberate restriction to prevent resource consumption attacks
 *     (any process that can create symlinks could fill disk with xattr data on
 ...
 *     filesystem's handler.
 *
 * examples: lgetxattr("/path/symlink", "security.selinux", buf, sizeof(buf));  // Read SELinux label of symlink
 *   lgetxattr("/path/symlink", "trusted.overlay.opaque", buf, sizeof(buf));  // Read overlayfs attr
 *   lgetxattr("/path/symlink", "security.selinux", NULL, 0);  // Query attribute size
 *
 * notes: lgetxattr() is primarily used when you need to read extended attributes
 *   from symbolic links themselves rather than their targets. This is important
 *   for operations like backup/restore where symlink metadata must be preserved,
 *   or for security contexts on symbolic links (SELinux assigns security labels
 *   to symlinks independent of their targets).
 *
 *   The user.* namespace cannot be used with lgetxattr on symlinks due to
 *   historical security concerns. The restriction exists because unprivileged
 *   users could create symlinks in world-writable directories and attach
 *   arbitrary amounts of xattr data, potentially consuming significant disk
 *   space. Only security.* (for LSM labels) and trusted.* (for privileged
 *   metadata) work on symlinks.
 *
 *   Common use cases for lgetxattr():
 *   - Reading SELinux labels from symlinks (security.selinux)
 *   - Backup utilities preserving symlink metadata
 *   - OverlayFS metadata on symlinks (trusted.overlay.*)
 *
 *   The common pattern for reading an xattr of unknown size is:
 *   1. Call lgetxattr() with size=0 to get the current size
 *   2. Allocate a buffer of that size
 *   3. Call lgetxattr() again with the buffer
 *   Note: The attribute value may change between steps 1 and 3 if another
 *   process modifies it. Handle ERANGE by retrying with a larger buffer.
 *
 *   Historical note: A bug fix (commit 3a3e159dbfe4) added ESTALE retry
 *   handling to lgetxattr() for NFS filesystems.
 *
 * since-version: 2.4
 */
...
/**
 * sys_llistxattr - list extended attribute names without following symlinks
 * @pathname: Path to the file whose extended attributes to list
 * @list: User buffer to receive the list of attribute names
 * @size: Size of the user buffer in bytes
 *
 * long-desc: Retrieves the list of extended attribute names associated with
 *   the file specified by pathname. This syscall is identical to listxattr()
 *   except that if pathname refers to a symbolic link, llistxattr() retrieves
 *   the attribute names of the link itself, rather than the file it refers to.
 *
 ...
 *   prefix (e.g., "security.selinux", "trusted.overlay.opaque").
 *
 *   If size is 0 and list is NULL, returns the current size of the attribute
 *   name list without copying any data. This allows the caller to determine
 *   the required buffer size before allocating memory.
 ...
 *   - POSIX ACLs (system.posix_acl_*) are listed if present and supported
 *
 *   Note that the user.* namespace is NOT supported on symbolic links. Since
 *   llistxattr() operates on the symlink itself, user.* attributes will never
 *   appear in the list for symbolic links. This is a deliberate restriction to
 *   prevent resource consumption attacks, as any unprivileged user who can
 *   create symlinks could otherwise attach arbitrary xattr data to them.
 *
 *   The maximum size of the attribute name list is limited to XATTR_LIST_MAX
 *   (65536 bytes). If the actual list exceeds this limit, E2BIG is returned.
 *
 ...
 * error: ENOENT, File not found
 *   desc: The file or symbolic link specified by pathname does not exist, or a
 *     directory component in the path does not exist. Note that the final
 *     component is not required to exist as a regular file - llistxattr can
 *     operate on symbolic links, but the link itself must exist.
 *
 * error: EACCES, Permission denied
 *   desc: Permission denied during path resolution (search permission denied on
 *     a directory component). Also returned by LSM security_inode_listxattr()
 ...
 *     the final path component is never followed, only preceding symlinks in
 *     the directory path are followed.
 *
 * error: ENAMETOOLONG, Filename too long
 *   desc: The pathname or a component of the pathname exceeds the system limit
 *     (PATH_MAX or NAME_MAX).
 ...
 * constraint: user.* namespace restrictions on symlinks
 *   desc: The user.* namespace is only supported on regular files and directories.
 *     Since llistxattr() operates on symbolic links directly, user.* attributes
 *     will never be listed for symlinks because symlinks cannot have them. This
 *     restriction exists to prevent resource consumption attacks where
 *     unprivileged users could create symlinks in world-writable directories
 *     and attach arbitrary amounts of xattr data.
 *
 * constraint: LSM checks
 *   desc: Linux Security Modules (SELinux, Smack, AppArmor) may restrict
 *     which attributes are visible via security_inode_listxattr() hook.
 ...
 *     handle this by retrying with a larger buffer.
 *
 * examples: llistxattr("/path/symlink", buf, sizeof(buf));  // List symlink xattrs
 *   llistxattr("/path/symlink", NULL, 0);  // Query required buffer size
 *
 * notes: llistxattr() is primarily used when you need to list extended attributes
 *   of symbolic links themselves rather than their targets. This is important
 *   for operations like backup/restore where symlink metadata must be preserved,
 *   or for listing security contexts on symbolic links (SELinux assigns security
 *   labels to symlinks independent of their targets).
 *
 *   The common pattern for listing extended attributes is:
 *   1. Call llistxattr() with size=0 to get required buffer size
 *   2. Allocate a buffer of that size
 *   3. Call llistxattr() again with the buffer
 *   4. Parse the returned list by scanning for null terminators
 *
 *   Be aware that:
 *   - The attribute list may change between calls (handle ERANGE by retrying)
 *   - trusted.* attributes are hidden from unprivileged callers
 *   - user.* attributes are never present on symlinks (by design)
 *   - Some filesystems may return an empty list even if they don't support xattrs
 *   - The returned names include namespace prefixes (user., trusted., etc.)
 *
 *   Common use cases for llistxattr():
 *   - Listing SELinux labels on symlinks (security.selinux)
 *   - Backup utilities enumerating symlink metadata
 *   - OverlayFS metadata enumeration on symlinks (trusted.overlay.*)
 *
 *   Related syscalls:
 *   - listxattr(): Same but follows symbolic links
 *   - flistxattr(): Operates on open file descriptor
 *   - lgetxattr(): Retrieve value of a specific attribute from symlink
 *   - lsetxattr(): Set value of an attribute on symlink
 *   - lremovexattr(): Remove an attribute from symlink
 *
 * since-version: 2.4
 */
...
/**
 * sys_removexattr - Remove an extended attribute from a file
 * @pathname: Path to the file from which to remove the attribute
 ...
 *   specified by pathname. Extended attributes are name:value pairs associated
 *   with inodes (files, directories, symbolic links, etc.) that extend the
 *   normal attributes (stat data) associated with all inodes.
 *
 *   The attribute name must include a namespace prefix. Valid namespaces are:
 *   - "user." - User-defined attributes (regular files and directories only)
 *   - "trusted." - Trusted attributes (requires CAP_SYS_ADMIN)
 ...
 *   - "system." - System attributes (e.g., POSIX ACLs via system.posix_acl_access)
 *
 *   This syscall follows symbolic links. Use lremovexattr() to operate on the
 *   symbolic link itself, or fremovexattr() to operate on an open file descriptor.
 *
 *   For POSIX ACL attributes (system.posix_acl_access, system.posix_acl_default),
 ...
 *     The path is resolved following symbolic links. Maximum path length is
 *     PATH_MAX (4096 bytes). The file must exist and the caller must have
 *     appropriate permissions to modify extended attributes.
 *
 * param: name
 *   type: KAPI_TYPE_USER_PTR
 *   flags: KAPI_PARAM_IN | KAPI_PARAM_USER
 ...
 *     never set or was already removed. Returned from the filesystem's xattr
 *     handler set operation with NULL value and XATTR_REPLACE flag.
 *
 * error: ENOENT, File not found
 *   desc: The file specified by pathname does not exist, or a directory component
 *     in the path does not exist. Returned from path lookup (filename_lookup).
 ...
 *   desc: Permission denied during path resolution (search permission on a directory
 *     component) or write access to the file is denied based on DAC permissions.
 *     Returned from inode_permission() during path resolution.
 *
 * error: EPERM, Operation not permitted
 *   desc: Returned in several cases: (1) The file is marked immutable (chattr +i)
 *     or append-only (chattr +a) - checked by may_write_xattr(). (2) For trusted.*
 *     namespace, caller lacks CAP_SYS_ADMIN in the filesystem's user namespace -
 *     checked by xattr_permission(). (3) For security.* namespace attributes
 *     (except security.capability), caller lacks CAP_SYS_ADMIN - checked by
 *     cap_inode_removexattr(). (4) For security.capability, caller lacks
 *     CAP_SETFCAP - checked by cap_inode_removexattr() via capable_wrt_inode_uidgid().
 *     (5) For user.* namespace on sticky directories, caller is not the owner
 *     and lacks CAP_FOWNER - checked by xattr_permission(). (6) The inode has
 *     an unmapped ID in an idmapped mount - checked by HAS_UNMAPPED_ID().
 *
 * error: EOPNOTSUPP, Operation not supported
 *   desc: The filesystem does not support extended attributes (IOP_XATTR not set
 ...
 *     underlying set_posix_acl() operation is not supported.
 *
 * error: ERANGE, Name out of range
 *   desc: The attribute name is empty (zero length) or exceeds XATTR_NAME_MAX
 *     (255 characters). Returned from import_xattr_name() via strncpy_from_user().
 ...
 * error: ELOOP, Too many symbolic links
 *   desc: Too many symbolic links were encountered during path resolution
 *     (more than MAXSYMLINKS, typically 40). Returned from filename_lookup().
 *
 * error: ENAMETOOLONG, Filename too long
 *   desc: The pathname or a component of the pathname exceeds the system limit
 ...
 *     on the same inode. The lock may be released and re-acquired if delegation
 *     breaking requires waiting.
 *
 * lock: sb->s_writers (superblock freeze protection)
 *   type: KAPI_LOCK_SEMAPHORE
 *   acquired: true
 ...
 *     during delegation breaking in __break_lease(). The syscall may wait for
 *     delegation holders to acknowledge the break. This occurs in the retry loop
 *     of vfs_removexattr() when try_break_deleg() indicates a delegation exists.
 *
 * signal: Any
 *   direction: KAPI_SIGNAL_RECEIVE
 *   action: KAPI_SIGNAL_ACTION_RESTART
 ...
 *     The change is typically persisted to storage synchronously or asynchronously
 *     depending on filesystem and mount options. For POSIX ACL removal, the file's
 *     permission mode may also be affected.
 *   reversible: yes
 *   condition: Operation succeeds
 *
 * side-effect: KAPI_EFFECT_MODIFY_STATE
 ...
 *   desc: On success, fsnotify_xattr() is called to notify any registered
 *     watchers (inotify, fanotify) of the extended attribute modification.
 *     This generates an IN_ATTRIB event for the file.
 *   condition: Operation succeeds
 *
 * side-effect: KAPI_EFFECT_MODIFY_STATE
 ...
 *     SELinux inode security data when security labels are removed.
 *   condition: Operation succeeds
 *
 * state-trans: extended attribute
 ...
 *     not existing on the file. The attribute name no longer appears in listxattr
 *     output and subsequent getxattr calls return ENODATA.
 *
 * capability: CAP_SYS_ADMIN
 *   type: KAPI_CAP_GRANT_PERMISSION
 ...
 *     set and provide xattr handlers). Common filesystems supporting xattrs
 *     include ext4, XFS, Btrfs, and tmpfs. Some filesystems (e.g., FAT, older
 *     ext2) do not support extended attributes and return EOPNOTSUPP.
 *
 * constraint: user.* namespace restrictions
 *   desc: The user.* namespace is only supported on regular files and directories.
 *     Attempting to remove user.* attributes on other file types (symlinks,
 *     devices, sockets, FIFOs) returns EPERM. This is checked by xattr_permission().
 *
 * constraint: LSM checks
 *   desc: Linux Security Modules (SELinux, Smack, AppArmor) may impose additional
 ...
 *
 * examples: removexattr("/path/file", "user.comment");  // Remove user attribute
 *   removexattr("/path/file", "trusted.overlay.opaque");  // Remove trusted attr
 *   removexattr("/path/file", "security.selinux");  // Remove SELinux label
 *
 * notes: Extended attributes provide a way to associate arbitrary metadata with
 *   files beyond the standard stat attributes. removexattr() is the complement
 *   to setxattr() - while setxattr creates or modifies attributes, removexattr
 *   deletes them entirely.
 *
 *   The trusted.* namespace is commonly used by overlayfs for opaque directory
 *   markers and by backup utilities to store metadata that should not be
 *   visible to unprivileged processes.
 *
 *   Removing the security.capability attribute removes any file capabilities
 *   from an executable, causing it to run with only the capabilities it
 ...
 *   This allows filesystems to implement removal through the same code
 *   path as setting attributes.
 *
 *   Unlike removexattr() which follows symlinks, lremovexattr() operates
 *   on the symbolic link itself, and fremovexattr() operates on an open
 *   file descriptor. The removexattrat() syscall (added in Linux 6.17)
 *   provides additional flexibility with AT_FDCWD and AT_* flags.
 *
 * since-version: 2.4
 */
