...
static int erofs_try_to_free_all_cached_pages(struct erofs_sb_info *sbi,
				       struct z_erofs_pcluster *pcl)
{
	unsigned int pclusterpages = z_erofs_pclusterpages(pcl);
	int i;
...
static void z_erofs_try_to_claim_pcluster(struct z_erofs_frontend *f)
{
	struct z_erofs_pcluster *pcl = f->pcl;
	struct z_erofs_pcluster **owned_head = &f->head;

	/* type 1, nil pcluster (this pcluster doesn't belong to any chain.) */
	if (!cmpxchg(&pcl->next, NULL, *owned_head)) {
		*owned_head = pcl;
...
static int z_erofs_pcluster_begin(struct z_erofs_frontend *fe)
{
	...
	if (!(map->m_flags & EROFS_MAP_META)) {
		...
		ptr = erofs_read_metabuf(&map->buf, sb, blknr, EROFS_NO_KMAP);
		if (IS_ERR(ptr)) {
					  ...
					  struct z_erofs_pcluster *pcl)
{
	 ...
	 */
	if (erofs_try_to_free_all_cached_pages(sbi, pcl))
		return false;
...
static int z_erofs_do_read_page(struct z_erofs_frontend *fe,
				       ...
				       unsigned int rabytes)
{
	...
	return (syncmode == EROFS_SYNC_DECOMPRESS_AUTO && !rabytes) ||
				     ...
				     struct page **pagepool)
{
	...
	for (; be.pcl != Z_EROFS_PCLUSTER_TAIL; be.pcl = next) {
		...
		z_erofs_decompress_pcluster(&be, err);
	}
}
...
out_tocache:
	if (!tocache || bs != PAGE_SIZE ||
	    add_to_page_cache_lru(page, mc, pcl->index + nr, gfp)) {
...
static void z_erofs_runqueue(struct z_erofs_frontend *f,
		...
		struct readahead_control *rac, bool backmost)
{
	...
	if (backmost) {
		...
		if (err || (map->m_flags & EROFS_MAP_FRAGMENT))
			return;
...
static void z_erofs_readahead(struct readahead_control *rac)
{
	...
	z_erofs_runqueue(&f, z_erofs_is_sync_decompress(sbi, nrpages << PAGE_SHIFT),
			 true);
	erofs_put_metabuf(&f.map.buf);
	erofs_release_pages(&f.pagepool);
}
