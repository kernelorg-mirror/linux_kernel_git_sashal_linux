...
MODULE_PARM_DESC(fw_log_level,
		 "VPU firmware default log level: debug=" __stringify(IVPU_FW_LOG_DEBUG)
...
{
	fw_log_print_all_in_bo(vdev, "VPU critical", vdev->fw->mem_log_crit, only_new_msgs, p);
	fw_log_print_all_in_bo(vdev, "VPU verbose", vdev->fw->mem_log_verb, only_new_msgs, p);
}
