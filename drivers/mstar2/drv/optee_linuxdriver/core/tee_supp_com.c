...
#ifdef CONFIG_MSTAR_CHIP
DECLARE_WAIT_QUEUE_HEAD(wait_tee_supp);
EXPORT_SYMBOL(wait_tee_supp);
#endif
				  ...
				  uint32_t id, void *data, size_t datalen)
{
	...
	if (atomic_read(&rpc->used) == 0) {
		...
		add_wait_queue(&wait_tee_supp, &wait);
#endif
		dev_err(tee->dev, "\033[1;33m%s: ERROR Supplicant application NOT ready[%d] ... wait\033[m\n"
				, __func__, task->tgid);
#ifdef CONFIG_MSTAR_CHIP
		...
		set_current_state(TASK_RUNNING);
		remove_wait_queue (&wait_tee_supp, &wait);
		dev_err(tee->dev, "\033[1;35mSupplicant application ready !!![%d]\033[m\n",  task->tgid);
#else
		goto out;
#endif
