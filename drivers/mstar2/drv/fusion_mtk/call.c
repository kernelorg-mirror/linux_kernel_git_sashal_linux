...
/* DirectFB define FUSION_CALL_RETURN_DATA_MAX limit */
                    ...
                    FusionCallExecute * execute)
{
     ...
     if(call == NULL)
     {
         FUSION_DEBUG( "%s line %d -> call is NULL\n", __FUNCTION__,__LINE__ );
         return -ENODATA;
     }
     ...
     if (fusionee && !(execute->flags & FCEF_ONEWAY)) {
          ...
          execution->is_executed_return = false;
     }
     ...
     if(call->fusionee == NULL)
     {
         FUSION_DEBUG( "%s line %d -> call->fusionee is NULL\n", __FUNCTION__,__LINE__ );
         return -ENODATA;
     }
     ...
     if (execution) {
          ...
          do {
              int time = 5 * HZ;
              fusion_core_wq_wait( fusion_core, &execution->wait, &time );
              if (time == 0) {
                    printk(KERN_EMERG "[%s-%d] timeout (pid=%d, comm=%s)\n", __func__, __LINE__, fusion_core_pid( fusion_core ), current->comm);
                    bWaitAgain = true;
              } else
                    bWaitAgain = false;
          } while (bWaitAgain);

          if (signal_pending(current) || execution->is_executed_return == false)
          {
                FUSION_DEBUG( "  -> woke up, SIGNAL PENDING!\n" );

                // execute->serial = execution->serial;
                printk( KERN_EMERG "FusionIPC: Warning signal_pending(current) -> exit from fusion_call_execute() without FCEF_RESUMABLE. current->pid=%d execution->is_executed_return=%d\n", current->pid, execution->is_executed_return );
                ...
                if (!(execute->flags & FCEF_RESUMABLE)) {
                     ...
                     /* call maybe be freed, call->fusionee->id will crash */
                     if(call && call->fusionee) {
                             printk( KERN_EMERG "fusion_call_execute: call->fusionee->id=%ld\n", call->fusionee->id );
                     }
                }
          ...
          bool is_executed_return_temp = execution->is_executed_return;
          ...
          fusion_skirmish_reclaim_all_ext(dev, fusion_core_pid( fusion_core ), is_executed_return_temp);
     }
     else {
...
{
     int ret;
     FusionCall *call = NULL, *next = NULL;
     FusionCallExecution *execution = NULL;
     ...
     direct_list_foreach_safe (execution, next, call->executions) {
          ...
          execution->is_executed_return = true;
     ...
     if (execution) {
          ...
          if(signal_pending(current)) 
          {
                ...
                if (!(execute->flags & FCEF_RESUMABLE)) 
                {
                     ...
                     /* call maybe be freed, call->fusionee->id will crash */
                     if(call && call->fusionee) {
                             printk( KERN_EMERG "fusion_call_execute3: call->fusionee->id=%ld\n", call->fusionee->id );
                     }
                }
...
fusion_call_return3(FusionDev * dev, int fusion_id, FusionCallReturn3 * call_ret)
{
     int ret;
     FusionCall *call = NULL;
     FusionCallExecution *execution = NULL, *next = NULL;
                                          ...
                                          unsigned int ret_size)
{
     ...
     /* if ret_size is large, maybe out of memory */
