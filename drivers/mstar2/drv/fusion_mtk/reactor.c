                        ...
                        Fusionee * fusionee, int msg_size, const void *msg_data)
{
     ...
     if (reactor->call_id) {
          ...
          if(!reactor->call_ptr && msg_size == sizeof(msg_data)) {
               /* fuzzy test crash: invalid-access */
               void *ptr = *(void **)msg_data;
               if((ulong)ptr >= FUSION_SHM_BASE &&
     ...
     fusion_list_foreach_safe(l, next, reactor->nodes) {
          if (l == NULL) {
              return 0;
          }
