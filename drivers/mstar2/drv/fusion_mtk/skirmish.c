...
struct __FUSION_FusionSkirmish {
     ...
     int lock_pid;
     char lock_comm[16];
     FusionSkirmish_Type skirmish_type;
     int lock_count;
     int wait_pid[5];
     char comm[5][16];
     int lock_total;
...
{
        const char *skirmish_type_table[] = {
            "FS_PREVAIL",
            "FS_DISMISS",
            "FS_TRANSFER_ALL_1",
            "FS_TRANSFER_ALL_2",
       ...
       printk(KERN_EMERG "[fusion] skirmish->lock_fid = %d\n", skirmish->lock_fid);
       printk(KERN_EMERG "[fusion] skirmish->lock_pid = %d\n", skirmish->lock_pid);
       printk(KERN_EMERG "[fusion] skirmish->lock_count = %d\n", skirmish->lock_count);
       printk(KERN_EMERG "[fusion] skirmish->lock_total = %d\n", skirmish->lock_total);
       printk(KERN_EMERG "[fusion] skirmish->notify_count = %d\n", skirmish->notify_count);
       printk(KERN_EMERG "[fusion] skirmish->lock_time = %lu\n", skirmish->lock_time);
       printk(KERN_EMERG "[fusion] skirmish->transfer_to = %llu\n", skirmish->transfer_to);
       printk(KERN_EMERG "[fusion] skirmish->transfer_from = %llu\n", skirmish->transfer_from);
       printk(KERN_EMERG "[fusion] skirmish->transfer_from_pid = %d\n", skirmish->transfer_from_pid);
       printk(KERN_EMERG "[fusion] skirmish->transfer_count = %d\n", skirmish->transfer_count);
       printk(KERN_EMERG "[fusion] skirmish->transfer_serial = %d\n", skirmish->transfer_serial);
       printk(KERN_EMERG "[fusion] skirmish->transfer2_to = %llu\n", skirmish->transfer2_to);
       printk(KERN_EMERG "[fusion] skirmish->transfer2_from = %llu\n", skirmish->transfer2_from);
       printk(KERN_EMERG "[fusion] skirmish->transfer2_from_pid = %d\n", skirmish->transfer2_from_pid);
       printk(KERN_EMERG "[fusion] skirmish->transfer2_count = %d\n", skirmish->transfer2_count);
       printk(KERN_EMERG "[fusion] skirmish->transfer2_serial = %d\n", skirmish->transfer2_serial);
       printk(KERN_EMERG "[fusion] skirmish->entry.waiters = %d\n",  skirmish->entry.waiters);
       printk(KERN_EMERG "[fusion] skirmish->entry.name = %s\n",  skirmish->entry.name);
       printk(KERN_EMERG "[fusion] skirmish->skirmish_type = %s\n",  skirmish_type_table[skirmish->skirmish_type]);
       printk(KERN_EMERG "[fusion] %s (pid=%d, comm=%s),parent (ppid=%d, comm=%s) \n", __func__, current->pid, current->comm,current->parent->pid,current->parent->comm);
}
...
fusion_skirmish_print(FusionEntry * entry, void *ctx, struct seq_file *p)
{
     ...
     seq_printf(p, ", c:%d, f:0x%08x, p:%d, waiters:%d type:%s entry.id:%d name:%s\n",
                skirmish->lock_count,
                skirmish->lock_fid,
                skirmish->lock_pid,
                skirmish->entry.waiters,
	 ...
	 if(skirmish->entry.waiters > 0) {
	seq_printf(p, ", wait0:%d, wait1:%d, wait2:%d, wait3:%d, wait4:%d\n",
			   skirmish->wait_pid[0],
			   skirmish->wait_pid[1],
			   skirmish->wait_pid[2],
			   skirmish->wait_pid[3],
	...
	seq_printf(p, ", wait0:%s, wait1:%s, wait2:%s, wait3:%s, wait4:%s\n",
			   skirmish->comm[0],
			   skirmish->comm[1],
			   skirmish->comm[2],
			   skirmish->comm[3],
...
int fusion_skirmish_prevail(FusionDev * dev, int id, int fusion_id)
{
     ...
     int i;
     bool bWaitAgain = false;
     unsigned int waitAgainCount = 0;
#ifdef FUSION_DEBUG_SKIRMISH_DEADLOCK
     FusionSkirmish *s;
     int i;
     bool outer = true;
     ...
     if (skirmish->lock_pid == fusion_core_pid( fusion_core )) {
          skirmish->lock_count++;
          skirmish->lock_total++;
          //printk(KERN_EMERG "[%s-%d](pid=%d, comm=%s) lock_pid=%d lock_comm=%s lock_count=%d\n", __func__, __LINE__, fusion_core_pid( fusion_core ), current->comm, skirmish->lock_pid, skirmish->lock_comm, skirmish->lock_count);
          return 0;
     }
#ifdef FUSION_DEBUG_SKIRMISH_DEADLOCK
     ...
     while (   skirmish->lock_pid
               ...
               || ( skirmish->skirmish_type == FS_RETURN_ALL_1  )
               || ( skirmish->skirmish_type == FS_RETURN_ALL_2  ) ) {


          static int i_temp=0;

          printk(KERN_EMERG "[%s-%d](pid=%d, comm=%s) lock_pid=%d lock_comm=%s type=%d i_temp=%d\n", __func__, __LINE__, fusion_core_pid( fusion_core ), current->comm, skirmish->lock_pid, skirmish->lock_comm, skirmish->skirmish_type, i_temp);

          skirmish->wait_pid[i_temp] = fusion_core_pid( fusion_core );
          memcpy(skirmish->comm[i_temp], current->comm, 16);
          i_temp++;
          if(i_temp == 5) {
              i_temp = 0;
          }
          ...
          do {
               int time = 5 * HZ;
               ret = fusion_skirmish_wait(skirmish, &time);
               if (ret == 0) {
                    break;
               } else if(ret != -ETIMEDOUT) {
                    printk(KERN_EMERG "[fusion][%s-%d] ret = %d skirmish->skirmish_type=%d lock_pid=%d\n", __func__, __LINE__, ret, skirmish->skirmish_type, skirmish->lock_pid);
                    return ret;
               }

               waitAgainCount++;
               printk(KERN_EMERG "[fusion][%s-%d](pid=%d, comm=%s) i_temp=%d waitAgainCount=%u\n", __func__, __LINE__, fusion_core_pid( fusion_core ), current->comm, i_temp, waitAgainCount);
               printk(KERN_EMERG "[fusion][%s-%d] ret = %d skirmish->skirmish_type=%d lock_pid=%d lock_comm=%s\n", __func__, __LINE__, ret, skirmish->skirmish_type, skirmish->lock_pid, skirmish->lock_comm);
               printk(KERN_EMERG "[fusion][%s-%d] wait0=%d 1=%d 2=%d 3=%d 4=%d\n", __func__, __LINE__, skirmish->wait_pid[0], skirmish->wait_pid[1], skirmish->wait_pid[2], skirmish->wait_pid[3], skirmish->wait_pid[4]);
               printk(KERN_EMERG "[fusion][%s-%d] wait0=%s 1=%s 2=%s 3=%s 4=%s\n", __func__, __LINE__, skirmish->comm[0], skirmish->comm[1], skirmish->comm[2], skirmish->comm[3], skirmish->comm[4]);
               printk(KERN_EMERG "[fusion][%s-%d] dispatcher transfer_to=%d transfer2_to=%d\n", __func__, __LINE__, fusionee_dispatcher_pid(dev, skirmish->transfer_to), fusionee_dispatcher_pid(dev, skirmish->transfer2_to));
               print_skirmish( skirmish );

               struct task_struct *task = NULL;
               struct pid *pid_struct = find_get_pid(skirmish->lock_pid);
               if(pid_struct) {
                    task = pid_task(pid_struct, PIDTYPE_PID);
               }

               if (task) {
                   printk (KERN_EMERG "[fusion] task->state=%d, task->pid=%d\n", task->state, task->pid);
               }
               else {
                   printk (KERN_EMERG "[fusion] lock_pid =%d, task is not running \n", skirmish->lock_pid);
               }

               if (task == NULL && (skirmish->lock_pid > 0)) {
                    printk (KERN_EMERG "[fusion]Process is not running \n");
                    fusion_skirmish_dismiss_all_from_pid(dev, skirmish->lock_pid);
                    bWaitAgain = false;
               } else if (skirmish->lock_pid == 0 && skirmish->skirmish_type == FS_DISMISS) {
                    bWaitAgain = false;
               } else {
                    bWaitAgain = true;
               }
     ...
     memcpy(skirmish->lock_comm, current->comm, 16);
     skirmish->skirmish_type = FS_PREVAIL;
     skirmish->lock_count = 1;
     skirmish->lock_time  = jiffies;
     //printk(KERN_EMERG "[%s-%d] lock_pid=%d lock_comm=%s\n", __func__, __LINE__, skirmish->lock_pid, skirmish->lock_comm);
     skirmish->lock_total++;
...
int fusion_skirmish_dismiss(FusionDev * dev, int id, int fusion_id)
{
     ...
     if (--skirmish->lock_count == 0) {
          ...
          skirmish->lock_fid = 0;
          skirmish->lock_pid = 0;
          skirmish->skirmish_type = FS_DISMISS;
                      ...
                      FusionID fusion_id)
{
     ...
     if (!wait->lock_count) {
          ...
          FUSION_DEBUG( "  -> lock_pid = 0\n" );
          printk(KERN_EMERG "[fusion][%s-%d] lock_pid=%d\n", __func__, __LINE__, skirmish->lock_pid);
     ...
     fusion_list_foreach(l, dev->skirmish.list) {
          ...
          if (skirmish->lock_pid == from_pid) {
               if (skirmish->transfer_to == 0) {
                    ...
                    skirmish->lock_fid   = 0;
                    skirmish->lock_pid   = 0;
                    skirmish->skirmish_type = FS_TRANSFER_ALL_1;
                    skirmish->lock_count = 0;
                    ...
                    skirmish->lock_fid   = 0;
                    skirmish->lock_pid   = 0;
                    skirmish->skirmish_type = FS_TRANSFER_ALL_2;
                    skirmish->lock_count = 0;
...
void fusion_skirmish_reclaim_all_ext(FusionDev * dev, int from_pid, bool is_executed_return_temp)
{
     FusionLink *l;
     ...
     fusion_list_foreach(l, dev->skirmish.list) {
          ...
          if ((skirmish->transfer2_to == 0)
              ...
              && (skirmish->transfer_from_pid == from_pid) ) {
               ...
               skirmish->skirmish_type = FS_RECLAIM_ALL_1;
               ...
               skirmish->skirmish_type = FS_RECLAIM_ALL_2;
               skirmish->transfer2_to       = 0;
               skirmish->transfer2_from     = 0;
               skirmish->transfer2_from_pid = 0;
               skirmish->transfer2_count    = 0;
